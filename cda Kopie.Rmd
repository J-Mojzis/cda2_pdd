...
kantone_japroz_mean["Kantone"] <- kantone_ju

japroz_graf <- kantone_japroz_mean
...
verschärfung <- ggplot(kantone_japroz_mean, aes(x=Kantone, 
...
resultat_pro <- ggplot(japroz_graf, 
                   aes(x=reorder(Kantone, -pro_verschärfung),y=pro_verschärfung))+
  geom_col(fill="gray")+
  geom_text(aes(label=round(pro_verschärfung, 1)), size = 3,
...
lockerung <- c("verschärfung","verschärfung","verschärfung","verschärfung",
               "verschärfung","verschärfung","verschärfung","verschärfung",
               "verschärfung","lockerung","verschärfung","verschärfung", 
               "verschärfung","verschärfung","verschärfung","verschärfung",
               "verschärfung","verschärfung","verschärfung","verschärfung",
               "verschärfung","lockerung","lockerung","lockerung","lockerung",
               "verschärfung")

ggplotly(verschärfung)
ggplotly(resultat_pro)

```
```{r}
new_data <- kantone_japroz_mean %>% filter(Position=="pro_verschärfung") %>% 
  rename(Kanton=Kantone)
```
...
kantone <- c("Zürich", "Bern", "Luzern", "Uri", "Schwyz", "Obwalden", 
             ...
             "Jura") 
kantone_ju <- c("Zürich", "Bern", "Luzern", "Uri", "Schwyz", "Obwalden", 
             "Nidwalden", "Glarus", "Zug", "Freiburg", "Solothurn", 
             "Basel-Stadt", "Basel-Land", "Schaffhausen","Appenzell A.Rh.", 
             "Appenzell I.Rh.", "Sankt Gallen", "Graubünden", "Aargau", 
             "Thurgau", "Tessin", "Waadt", "Wallis", "Neuenburg", "Genf") 
...
beteiligung_kanton <- ggplot(kantone_bet_graf, aes(x=reorder(Kanton, Beteiligung), 
...
bet_kantone <- ggplot(kantone_bet_graf, 
                      aes(x=reorder(Kanton, -Beteiligung), y=Beteiligung))+
  geom_col(fill="grey")+
  geom_text(aes(label=round(Beteiligung, 1)))+
  annotate("text", label=paste("Beteiligung der\nVolkabstimmung\n",round(volk_bet, 2),"%"), 
           x=20, y=60, size=4, colour="red")+
  geom_hline(yintercept = volk_bet, alpha=0.5, size = 0.5, colour="red")+
  theme(axis.text.x = element_text(angle = 90))+
  labs(x= "Kantone", 
       title = "Durchschnittliche kantonale Beteiligung bei Volksabstimmungen bezüglich \nFlüchtlinge und Ausländer (1922-2016)")+
  coord_flip()


ggplotly(bet_kantone)  
...
bet_volk <- totale_bet_mittel %>% filter(Abgabe!= "Enthaltung", 
                                         Kantone!="Volk")

bet_volk_graf <- ggplot(bet_volk, 
                      aes(x=reorder(Kantone, -Prozent), y=Prozent))+
  geom_col(fill="grey")+
  ...
  theme(axis.text.x = element_text(angle = 90))+
  labs(x= "Kantone", 
       title = "Durchschnittliche Beteiligung bei Volksabstimmungen bei allen Abstimmungen
...
ggplotly(bet_volk_graf)
```
#kanton vs total position dodge

```{r}
fl_kant <- kantone_bet_graf %>% 
  pivot_wider(
    names_from = "Abgabe", 
    values_from = "Prozent"
  ) %>% 
  rename(Flüchtlinge=Beteiligung)

fl_kant <- fl_kant[,-3]

tot_kant <- totale_bet_mittel[-c(1, 2),] %>% 
  pivot_wider(
    names_from = "Abgabe", 
    values_from = "Prozent"
  ) %>% 
  rename(Alle=Beteiligung,
         Kanton=Kantone)
tot_kant <- tot_kant[,-3]

tot_vs_kant <- left_join(fl_kant, tot_kant, by="Kanton")

dif <- tot_vs_kant
tot_vs_kant <- tot_vs_kant %>% 
  arrange(desc(Flüchtlinge)) %>% 
  pivot_longer(cols = c(2, 3),
               names_to = "Alle_vs_Flüchtlinge", 
               values_to = "Beteiligung")

vergleich <-  ggplot(tot_vs_kant, aes(x=Kanton, y=Beteiligung, fill=Alle_vs_Flüchtlinge))+
  geom_bar(position="dodge", stat = "identity")+
  theme(axis.text.x = element_text(angle = 90))+
  labs(x= "Kantone", 
       title = 
         "Differenz der Beteiligung von allen Abstimmungen vs.Flüchtlingsabstimmungen 
       im Zeitraum 1848-2022")



dif <- dif %>% 
  mutate(Tendenz=lockerung,
         Differenz=Flüchtlinge-Alle)

new_data_2 <- left_join(dif, new_data, by="Kanton") %>% filter(Kanton != "Jura")

new_data_graf <- ggplot(new_data_2, aes(reorder(Kanton, Differenz), Differenz, fill=Prozent))+
  geom_col()+
  labs(x="Kanton",
       title = "Differenz der Stimmbeteiligung bei\nFlüchtlingsthemen gegenüber allen Abstimmungen", 
       fill="Tendenz ± 50%")+
  geom_hline(yintercept = round(mean(dif$Differenz),2))+
  geom_hline(yintercept = round(mean(dif$Differenz),2)+sd(dif$Differenz), 
             colour="grey")+
  geom_hline(yintercept = round(mean(dif$Differenz),2)-sd(dif$Differenz), 
             colour="grey")+
   annotate("text", label=paste(
     "Durchschnittliche Differenz bei\nFlüchtlingsthemen vs. alle Abstimmungen:\n",
                               round(mean(dif$Differenz),2),"%"), 
           x=8, y=4.8, size=4, colour="black")+
  coord_flip()+
...
dif_graf <- ggplot(dif, aes(reorder(Kanton, Differenz), Differenz, fill=Tendenz))+
  geom_col()+
  labs(x="Kanton",
       title = "Differenz der Stimmbeteiligung bei\nFlüchtlingsthemen gegenüber allen Abstimmungen", 
       fill="Tendenz ± 50%")+
...
ggplotly(vergleich)
ggplotly(dif_graf)
newgraf <- ggplotly(new_data_graf)
```
...
herkunft_line <- ggplot(cum_as_nat, aes(x=Year, y=Amount, group=Country))+
  geom_line()+
  theme(axis.text.x = element_text(angle = 90))+
  labs(x="Jahr", y="Anzahl", title = "Flüchtlingsherkunft seit 1986 \n20 häufigste Länder und alle Andere")

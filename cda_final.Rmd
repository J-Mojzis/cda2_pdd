...
data <- df_swissvotes %>%
  ...
  mutate(across(names(var_positionen), 
  ...
  mutate(across(names(var_hauptthema), 
  ...
  mutate(across(names(var_unterthema), 
    ...
    TRUE ~"andere")))) %>%
  mutate(across(names(var_unterunterthema), 
             ~ factor(case_when(. == 10.31 ~ "Ausländerpolitik",
    .== 10.32 ~ "Flüchtlinge",
     TRUE ~"andere")))) %>%
  
  mutate(across(names(var_datum), 
                ~ as.Date(., "%d.%m.%Y"))) %>%
  
  mutate(across(names(var_resultate), 
...
df <- data %>%
  select(anr,Jahr, Kurztitel, rechtsform, annahme, bet_volkja.proz, bet,  d1e1:br.pos, bv.pos:nr.pos, sr.pos, unter_g, unter_u, ends_with(var_parteien), ja.lager, nein.lager, volk:stand, volkja.proz, ktjaproz,matches("...bet"),matches("...japroz"), matches("...annahme"),swissvoteslink, anneepolitique, info_br.de, info_dep.de, info_amt.de)
...
df_ausländer_korr <- select(df_ausländer, -c(names(df_japroz_ausländer))) %>% bind_cols(df_japroz_ausländer)
...
colnames(df_einwanderungen) <- c("Jahr","Bilanz", "Einwanderung", "Auswanderung", "Saldo", "Einbürgerungen")
...
df_bevoelkerung_total  <- inner_join(df_einwanderungen, df_bevoelkerung) %>% mutate(Anteil = Einbürgerungen * 100 / Bilanz, Anteil_ausl = Bilanz * 100 / Bevölkerung)
...
(graf_anteil_ausländer <- ggplotly(ggplot(df_bevoelkerung_total)+
  ...
  theme_minimal()+
  labs(title = "Anteil Ausländer in der Bevölkerung",
       y = "Anteil in %")
  ))
...
df_positionen_total <- inner_join(df_positionen_ausl_pivoted, df_positionen_pivoted, by="Kanton", "br.pos")
...
df_pro <- select(df, ge.japroz, ne.japroz,vs.japroz, vd.japroz, fr.japroz, bs.japroz, gr.japroz) %>% drop_na(.)
df_anti <- select(df, ag.japroz, gl.japroz,nw.japroz, so.japroz,tg.japroz, sz.japroz) %>% drop_na(.)

df_pro_ausländer <- select(df_ausländer, ge.japroz, ne.japroz,vs.japroz, vd.japroz, fr.japroz, bs.japroz, gr.japroz) %>% drop_na(.)
df_anti_ausländer <- select(df_ausländer, ag.japroz, gl.japroz,nw.japroz, so.japroz,tg.japroz, sz.japroz) %>% drop_na(.)
...
make_korrplot <- function(data){
 ...
 scale_fill_gradient2(low = "green", high = "dark blue", mid = "white", 
...
ggplotly(make_korrplot(df_pro_ausländer)) %>% layout(title = "Korrelationsmatrix pro Lockerungen, Ausländerfragen")
ggplotly(make_korrplot(df_pro))%>% layout(title = "Korrelationsmatrix pro Lockerungen, alle Abstimmungen")
ggplotly(make_korrplot(df_anti_ausländer)) %>% layout(title = "Korrelationsmatrix pro Verschärfungen, Ausländerfragen")
ggplotly(make_korrplot(df_anti))%>% layout(title = "Korrelationsmatrix pro Verschärfung, Alle Abstimmungen")
ggplotly(make_korrplot(df_japroz_ausländer))%>% layout(title = "Korrelationsmatrix pro Verschärfung, Alle Daten")
ggplotly(make_korrplot(df_japroz_alle))%>% layout(title = "Korrelationsmatrix pro-Verschärfung, Alle Daten")
...
(graf_bet_volk <- ggplotly(ggplot(df_bet_alle, 
                      aes(x=reorder(Kanton, -Prozent), y=Prozent))+
  geom_col(fill="grey")+
  geom_text(aes(label=round(Prozent, 1)))+
  annotate("text", label=paste("Durchschn. Beteiligung\n Gesamtschweizerisch:",round(mean_bet, 2),"%"), 
           x=20, y=60, size=4, colour="red")+
...
df_bet_japroz <- inner_join(df_bet_diff,df_japroz_kantone_ausl, by = "Kanton" ) %>% 
  select(-(starts_with("Abgabe"))) %>% 
  mutate(Differenz = Prozent.y - Prozent.x) %>% 
  rename(bet_allg = Prozent.x,
         bet_ausl = Prozent.y)
...
(graf_differenz <- (ggplot(df_bet_japroz, aes(reorder(Kanton, Differenz), Differenz, fill=japroz))+
  geom_col()+
  labs(x="Kanton",
       title = "Differenz der Stimmbeteiligung bei\nFlüchtlingsthemen gegenüber allen Abstimmungen", 
       fill="Tendenz ± 50%")+
  geom_hline(yintercept = round(mean_dif,2)+sd(df_bet_japroz$Differenz), 
             colour="grey")+
  geom_hline(yintercept = round(mean_dif,2)-sd(df_bet_japroz$Differenz), 
             colour="grey")+
  coord_flip()+
...
(graf_bet_ausländer <- ggplotly(ggplot(df_stimmbet_ausl, aes(bet, anr,fill= annahme))+
  geom_col()+
  labs(title = "Durchschnittliche Stimmbeteiligung für Vorlagen mit Thema Ausländer",
       y = "Vorlage-Nr",
       x = "Beteiligung in %")+
  geom_text(aes(label=abs(round(bet, 1))), size = 3,
...
rownames(df_japroz_mean_ausl) <- gsub(".japroz", "", rownames(df_japroz_mean_ausl), fixed=TRUE)
...
(graf_resultat_pro <- ggplotly(ggplot(df_japroz_mean_ausl, 
                   aes(x=reorder(Kantone, -Zustimmung),y=Zustimmung))+
  geom_col(fill="gray")+
  geom_text(aes(label=round(Zustimmung, 1)), size = 3,
    position = position_dodge(width = 1))+
  annotate("text", label = "Durchschnitt der\nVolksabstimmungen\n53.36%", 
           x=10, y=44, size=4, colour="dark blue")+
  geom_hline(yintercept = c(quantile[2], quantile[4]), alpha=0.5, size = 0.5)+
  geom_hline(yintercept = 50, alpha=0.5, size = 0.5, color = "dark blue")+
    geom_hline(yintercept = quantile[3], alpha=0.5, size = 0.5, color = "dark blue")+
  theme_minimal()+
  theme(axis.text.x = element_text(angle = 90))+
  labs(x="Kantone", y="Prozent", 
       title = "Durchschnittliches Zustimmung zu Verschärfungen bei Volksabstimmungen \nbezüglich Flüchtlinge und Ausländer (1922-2016)")))

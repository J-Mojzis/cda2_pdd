...
# Militär und Finanzierung
# Wie viele davon abgehehnt / angenommen?
```{r}
zahlen_ang_abg1 <- d2e1_6 %>%
  group_by(annahme) %>%
  count()
```
...
# Abstimmungereignisse im Bereich Medien
```{r}
d1es_12.5_anr <- subset(d1e2_12.5, anr == 181 | anr == 595 | anr ==617 | anr ==654)
```
```{r}
zahlen_ang_abg2$annahme[zahlen_ang_abg2$annahme == "0"] <- "Abgelehnt"
zahlen_ang_abg2$annahme[zahlen_ang_abg2$annahme == "1"] <- "Angenommen"
```
```{r}
ggplot(zahlen_ang_abg2, aes(x="", y=n, fill=annahme))+
  ...
  scale_fill_manual(values=c("tan2", "#0C4C8A"))+
    labs(
    title = "Abstimmungereignisse im Bereich Medien")
```

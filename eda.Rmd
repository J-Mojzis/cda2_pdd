...
svd_original <- read_csv2("swissvotes_dataset.csv")
```
...
ggplot(svd, aes(year, annahme)) + 
  ...
  scale_x_discrete(breaks = seq(1850, 2022, 10)) +
  labs(y="Politikbereich") +
...
```{r}
svd_einleitung <- filter(svd_na, d1e1 == "2" | d1e1 == "3" | d1e1 == "10" | d1e3 == "10.3.2")

d1e1.labs <- c("Aussenpolitik", "Sicherheitspolitik", "Ausländerpolitik und\nFlüchtlinge")
names(d1e1.labs) <- c("2", "3", "10")


ggplot(svd_einleitung, aes(jahr, annahme, colour = d1e1)) + 
  geom_jitter() + 
  facet_grid(rows = vars(d1e1), labeller = labeller(d1e1 = d1e1.labs)) + 
  theme(axis.ticks.y = element_blank(), 
        axis.text.y = element_blank()) #+
#  scale_x_discrete(breaks = seq(1850, 2022, 10)) +
#  labs(y="Politikbereich")
